=== SKT Trekking ===

Contributors: sonalsinha21
Requires at least: 5.0
Requires PHP:  5.6
Tested up to: 6.2
Version: 1.0
License: GPL-2.0-or-later
License URI: https://www.gnu.org/licenses/license-list.html#GPLv2
Tags:two-columns,right-sidebar,full-width-template,custom-background,custom-colors,custom-menu,sticky-post,theme-options,threaded-comments,featured-images,footer-widgets,translation-ready,rtl-language-support,custom-logo,editor-style,custom-header,blog,e-commerce,portfolio

== Description ==

SKT Trekking is an adventure camp thrilling exciting sports adrenaline rush activities summer camping for kids mountaineering and other activities like travel, jungle safari, destination, holiday, tourism, tours, backpacking, camping, climbing, fishing, hiking, nature, outdoor, running, surfing, rafting, cruise, trip, itineraries, hunting, railing, skating, skiing, water games. Tour hotel restaurant operators food drink and bed and breakfast motels airlines vacation portals travel agents can also operate this kind of business to attract more tourists. SEO friendly, easy to use, flexible and scalable. WooCommerce plug and play for shop. Compatible with contact form for call to action. Booking plugins can also be integrated easily.
 
== Theme Resources == 

Theme is built using the following resource bundles:

* All js that have been used are within folder /js of theme.

*   SKT Themes Self Designed Images:
	skt-trekking/images/cart-icon.png
	skt-trekking/images/free-vs-pro.png
	skt-trekking/images/icon-fb.png
	skt-trekking/images/icon-in.png
	skt-trekking/images/icon-insta.png
	skt-trekking/images/icon-skt-templates.png
	skt-trekking/images/icon-tw.png
	skt-trekking/images/icon-youtube.png
	skt-trekking/images/logo.png
	skt-trekking/images/shopping-icon.png
	skt-trekking/images/sktskill.jpg
	
*   Other Images
	skt-trekking/images/footer-bg.jpg	
		 
		
= 	Declaring these self designed images under GPL license version 2.0 =
License URL: http://www.gnu.org/licenses/gpl-2.0.html

*       Details of images used in Screenshot:
		https://pxhere.com/en/photo/1279226
		https://www.maxpixel.net/Trekking-Mountain-Woods-Path-Trees-Hiking-Man-6002581
		
*       Other Images [skt-trekking/images/footer-bg.jpg]:
		https://www.maxpixel.net/Mountaineering-Mountains-Tibet-Snow-Mountain-Peak-7157189		

		= 	Declaring these self designed images under GPL license version 2.0 =
			License URL: http://www.gnu.org/licenses/gpl-2.0.html
		
		Image used from maxpixel.net and pxhere.com
		Both sites provides images under CC0 license
 		CC0 license: https://creativecommons.org/share-your-work/public-domain/cc0 		
		
*   skt-trekking/customize-pro	

		Customize Pro code based on Justintadlock’s Customizer-Pro 
		Source Code URL : https://github.com/justintadlock/trt-customizer-pro			
		License : http://www.gnu.org/licenses/gpl-2.0.html
		Copyright 2016, Justin Tadlock	justintadlock.com

= Dashicons =
* 		License: dashicons is licensed under GPLv2
* 		https://github.com/WordPress/dashicons	
		
= WPTT Webfont Loader =
		https://github.com/WPTT/webfont-loader
		License: Distributed under the terms of the MIT License (https://github.com/WPTT/webfont-loader/blob/master/LICENSE)
		Copyright: 2020 Ari Stathopoulos, WordPress Themes Team	
				 
        
For any help you can mail us at support[at]sktthemes.com

== Changelog ==
= 1.0 =
*      Initial version release. 		