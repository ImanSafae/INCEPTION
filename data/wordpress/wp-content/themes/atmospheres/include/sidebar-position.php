...
add_action( 'customize_register', 'atmospheres_sidebar_customize_register' );
function atmospheres_sidebar_customize_register( $wp_customize ) {
...
***********************************************************************************/
		...
		function customize_sanitize_sidebar( $input ) {
			$valid = array(
				'1' => __( 'Left', 'atmospheres' ),
				'2' => __( 'Right', 'atmospheres' ),
				'3' => __( 'No Sidebar', 'atmospheres' ),
			);
...
***********************************************************************************/
		...
		$wp_customize->add_section( 'atmospheres_sidebar' , array(
			'title'       => __( 'Sidebar Options', 'atmospheres' ),
			'description'       => __( 'The sidebar options can only be used if there are active widgets in the sidebar. Also make sure you do not use a page template because in some case they take precedence over the sidebar position. Sidebar position for single page or post is available in the premium version of the theme.', 'atmospheres' ),
			'priority'   => 64,
		) );
	
		$wp_customize->add_setting( 'atmospheres_sidebar_position_home', array (
			'sanitize_callback' => 'customize_sanitize_sidebar',
		) );
				
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'atmospheres_sidebar_position_home', array(
			'label'    => __( 'Sidebar Position Home Page', 'atmospheres' ),
			'section'  => 'atmospheres_sidebar',
			'settings' => 'atmospheres_sidebar_position_home',
			'type' => 'radio',
			'choices' => array(
				'1' => __( 'Left', 'atmospheres' ),
				'2' => __( 'Right', 'atmospheres' ),
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'sidebar_icons', array(
			'label'    => __( 'Deactivate All Widget Icons', 'atmospheres' ),
			'section'  => 'atmospheres_sidebar',
			'priority'    => 13,				
		...
		$wp_customize->add_setting( 'atmospheres_sidebar_position_blog', array (
			'sanitize_callback' => 'customize_sanitize_sidebar',
		) );
				
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'atmospheres_sidebar_position_blog', array(
			'label'    => __( 'Sidebar Position Blog and Archive Pages', 'atmospheres' ),
			'section'  => 'atmospheres_sidebar',
			'settings' => 'atmospheres_sidebar_position_blog',
			'type' => 'radio',
			'choices' => array(
				'1' => __( 'Left', 'atmospheres' ),
				'2' => __( 'Right', 'atmospheres' ),
		...
		$wp_customize->add_setting( 'atmospheres_sidebar_position_categories', array (
			'sanitize_callback' => 'customize_sanitize_sidebar',
		) );
				
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'atmospheres_sidebar_position_categories', array(
			'label'    => __( 'Sidebar Position Categories', 'atmospheres' ),
			'section'  => 'atmospheres_sidebar',
			'settings' => 'atmospheres_sidebar_position_categories',
			'type' => 'radio',
			'choices' => array(
				'1' => __( 'Left', 'atmospheres' ),
				'2' => __( 'Right', 'atmospheres' ),
		...
		$wp_customize->add_control( new WP_Customize_Range_Control( $wp_customize, 'sidebar_width', array(
			...
			'priority' => 9,
			'section' => 'atmospheres_sidebar',
			'label'       => __( 'Sidebar Width in % ', 'atmospheres' ),
			'input_attrs' => array(
				'min'  => 20,
				'max'  => 30,
				'step' => 1,
 ...
 *   Styles
 */ 
add_action( 'wp_enqueue_scripts', 'atmospheres_sidebar_styles' );
function atmospheres_sidebar_styles() {
        $sidebar_position_home = esc_html( get_theme_mod( 'atmospheres_sidebar_position_home' ) );
        $sidebar_icons = esc_html( get_theme_mod( 'sidebar_icons' ) );
        $sidebar_position_blog = esc_html( get_theme_mod( 'atmospheres_sidebar_position_blog' ) );	
        $atmospheres_sidebar_position_categories = esc_html( get_theme_mod( 'atmospheres_sidebar_position_categories' ) );	
...
## Blog Pages Sidebar		
		...
		## Blog Pages Sidebar		
		if(((!is_singular() and $atmospheres_sidebar_position_categories == '2')  )  and is_category() and ( !is_front_page() or !is_home() )) { $categories = "
			#content #primary { float: left !important; padding-top: 0 !important; }
			#content #secondary {  float: right !important;  padding: 0 20px 20px 20px !important;}	
			
			";
		}
		else if (((!is_singular() and $atmospheres_sidebar_position_categories == '3') and is_category() and $atmospheres_sidebar_position_categories == '3') and ( !is_front_page() or !is_home() )) { $categories = "
			 #content #primary { width: 100% !important; float:none !important; padding-top: 0 !important; } 
			 #content #secondary { display: none !important; }
			";	
		}
		else if (((!is_singular() and $atmospheres_sidebar_position_categories == '1') ) and is_category() and ( !is_front_page() or !is_home() )) { $categories = "
			 #content #primary { padding-top: 0; } 
			";	
		}		
		else { 
		...
		if($sidebar_width) {
				if( 
					get_post_meta( get_the_ID(), 'atmospheres_metabox_sidebar', true ) != "no" ||
