...
add_action( 'customize_preview_init', 'score_customize_preview_js' );
	...
	function atmospheres__customize_register_menu( $wp_customize ) {
		...
		/**
			...
			* @return string
		*/
		function atmospheres_sanitization_alpha_colors( $value ) {
		...
		function atmospheres_menu_position_sanitize_position( $input ) {
			$valid = array(
			'left' => esc_attr__( 'Left', 'atmospheres' ),
			'right' => esc_attr__( 'Right', 'atmospheres' ),
			'center' => esc_attr__( 'Center', 'atmospheres' ),
			);
		...
		$wp_customize->selective_refresh->add_partial( 'menu_font_size', array(
			...
			'render_callback' => 'atmospheres_customize_partial_menu_font_size',
		) );		
		
		$wp_customize->add_section( 'seos_menu_section' , array(
			'title'       => __( 'Menu Options', 'atmospheres' ),
			'priority'    => 24,	
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'hide_menu', array(
			'label'    => __( 'Hide Menu Top', 'atmospheres' ),
		...
		$wp_customize->add_setting( 'atmospheres_header_search', array (
		'default' => '',		
		'sanitize_callback' => 'atmospheres__sanitize_checkbox',
		) );
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'atmospheres_header_search', array(
		'label'    => __( 'Activate search', 'atmospheres' ),
		'section'  => 'seos_menu_section',
		'priority'    => 1,				
		'settings' => 'atmospheres_header_search',
		'type' => 'checkbox',
		) ) );
		
		if( class_exists( 'WooCommerce' ) ) {		
			$wp_customize->add_setting( 'atmospheres_header_cart', array (
			'default' => '',		
			'sanitize_callback' => 'atmospheres__sanitize_checkbox',
			) );
			$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'atmospheres_header_cart', array(
			'label'    => __( 'Activate WooCommerce Cart', 'atmospheres' ),
			'section'  => 'seos_menu_section',
			'priority'    => 1,				
			'settings' => 'atmospheres_header_cart',
			'type' => 'checkbox',
			) ) );
		}	
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'menu_position_absolute', array(
			'label'    => __( 'Deactivate menu top position absolute', 'atmospheres' ),
			'description'    => __( 'The option is active only if has header image or video.', 'atmospheres' ),
			'section'  => 'seos_menu_section',
			'priority'    => 1,				
		...
		$wp_customize->add_setting( 'menu_position', array (
			'sanitize_callback' => 'atmospheres_menu_position_sanitize_position',
			'default'  => 'right'	
		) );
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'menu_position', array(
			'label'    => __( 'Menu Top Position', 'atmospheres' ),
			'section'  => 'seos_menu_section',		
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'menu_font_size', array(
			...
			'label'       => __( 'Menu Top Font Size', 'atmospheres' ),			
			'type'     =>  'number',
			'input_attrs'     => array(
			'min'  => 11,
			'max'  => 30,
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'menu_weight', array(
			'label'    => __( 'Menu Top Font Weight', 'atmospheres' ),
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'menu_font', array(
			'label'    => __( 'Menu Top Font', 'atmospheres' ),
			'section'  => 'seos_menu_section',
			'priority'    => 1,
			'settings' => 'menu_font',
			'type'     =>  'select',
		...
		******************************/		
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'menu_transform', array(
			'label'    => __( 'Menu Top Text Transform', 'atmospheres' ),
			'section'  => 'seos_menu_section',
		...
		$wp_customize->add_setting(
			...
			'sanitize_callback' => 'atmospheres_sanitization_alpha_colors',
			)
		);
		...
		array(
		'label'        => __( 'Menu Top Background', 'atmospheres' ),
		'description'        => __( 'When there is a header image and menu position absolute', 'atmospheres' ),
		'section'      => 'seos_menu_section',
		'priority' => 14,
		...
		'label'        => __( 'Menu Top Background', 'atmospheres' ),
		'description'        => __( 'When there is no header image', 'atmospheres' ),
		'priority' => 14,
		'section'  => 'seos_menu_section',
		...
		'label'    => __( 'Menu Top Color', 'atmospheres' ),
		'priority' => 14,
		'section'  => 'seos_menu_section',
		...
		'label'    => __( 'Menu Top Hover Color', 'atmospheres' ),
		'priority' => 14,
		'section'  => 'seos_menu_section',
		...
		'label'    => __( 'Sub Menu Top Color', 'atmospheres' ),
		'priority' => 14,
		'section'  => 'seos_menu_section',
		...
		'label'    => __( 'Sub Menu Top Hover Color', 'atmospheres' ),
		'priority' => 14,
		'section'  => 'seos_menu_section',
		...
		'label'    => __( 'Sub Menu Top Border Top Color', 'atmospheres' ),
		'priority' => 14,
		'section'  => 'seos_menu_section',
		...
		'label'    => __( 'Sub Menu Top Background Color', 'atmospheres' ),
		'priority' => 14,
		'section'  => 'seos_menu_section',
		...
		'label'    => __( 'Sub Menu Top Background Hover Color', 'atmospheres' ),
		'priority' => 14,
		'section'  => 'seos_menu_section',
	...
	add_action( 'customize_register', 'atmospheres__customize_register_menu' );
	/********************************************
		* Content Styles
	*********************************************/ 	
	
	function atmospheres__menu_styles () {
        ...
        $menu_background_image = esc_html(get_theme_mod( 'menu_background_image','rgba(10,10,10,0.6)' ) );
		...
		else if( get_post_meta( get_the_ID(),'atmospheres_value_header_image', true ) and has_post_thumbnail() and ( (get_theme_mod( 'header_image_show' ) == 'all' or get_theme_mod( 'header_image_show', 'home' ) == 'home') ) and !get_theme_mod('menu_position_absolute') ) { 
			$absolute = ".menu-cont{ position: absolute; }";
			if( $menu_background_image ) { $menu_background_image_style = "@media screen and (min-width: 800px) {body .menu-cont { background: {$menu_background_image};}}";} else {$menu_background_image_style ="";}
		}		
...
add_action( 'wp_enqueue_scripts', 'atmospheres__menu_styles' );